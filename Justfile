...
coverage: (build "debug")
    ...
    grcov target/coverage engine/target/coverage chess/target/coverage engine-cli/target/coverage -s . \
        --binary-path ./target/debug/ --output-types lcov -o ./target/coverage/byte-knight.lcov \
        --branch --keep-only "engine/*" --keep-only "chess/*" --keep-only "engine-cli/*" \
...
purge-coverage:
    ...
    rm -rf engine-cli/target/coverage/*.profraw
