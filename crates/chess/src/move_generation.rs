...
/// A directional bitboard shift (e.g. [`bitboard_helpers::north`]).
pub(crate) type ShiftFn = fn(Bitboard) -> Bitboard;

/// Side-dependent pawn shift helpers: forward push, capture toward the lower
/// file ("left"), capture toward the higher file ("right"), and the third rank
/// (where a legal single push from the start rank lands).
pub(crate) fn pawn_shifts(us: Side) -> (ShiftFn, ShiftFn, ShiftFn, Bitboard) {
    match us {
        Side::White => (
...
/// directional shift and push them into the given [`MoveList`].
///
/// Each destination's origin is recovered by the inverse shift `origin`
/// (`(file_delta, rank_delta)`), which is exact because every destination came
/// from moving pawn on the [`Board`]. Destinations on the promotion rank expand
/// into the four [`PROMOTION_FLAGS`]; others use `quiet_flag`. `emit_promotions`
/// / `emit_quiets` gate the two cases so a single target set can serve
/// different [`MoveFilter`]s.
///
/// # Arguments
///
...
/// - `emit_quiets`: Flag to indicate if quiet moves should be included.
#[inline]
pub(crate) fn emit_pawn_targets(
    ...
    for to in targets {
        let from = to.offset_unchecked(origin.0, origin.1);
...
fn get_pawn_moves(board: &Board, move_list: &mut MoveList, move_filter: MoveFilter) {
    ...
    let (push, capture_left, capture_right, third_rank) = pawn_shifts(us);
