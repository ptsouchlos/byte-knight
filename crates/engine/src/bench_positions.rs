...
//! Standard set of benchmark positions shared by the search bench (`engine-cli`)
//! and the evaluation benchmark (`crates/engine/benches/eval.rs`), so both measure
//! over the same positions.

/// FEN strings for the standard benchmark suite.
///
